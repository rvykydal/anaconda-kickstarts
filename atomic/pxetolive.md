In this post I'll show you how to build live Atomic Host image that can be run on diskless machine using PXE boot, and how the live system can be further customized with boot options and cloud-config for better use of resources, leading to goal of storing Docker images on iSCSI devices.
...
* Download Atomic **installer iso** which will be used to install live Atomic to raw disk image in a virtual machine.
...
First it creates raw Atomic disk image by Anaconda installation using installer iso and kickstart in kvm.
...
Yes, seems like quite a bit of layers. I'll talk about another option which should be available in Fedora rawhide later.


Running live Atomic Host from PXE:
----------------------------------

So, let's move our kernel and initrd images to PXE server, live image to HTTP server, update PXE_CONFIG accordingly and set it up on PXE server, eg:
...
Here is example of [meta-data](https://github.com/rvykydal/anaconda-kickstarts/blob/master/atomic/ci/pxe-to-live-with-iscsi/meta-data) file.
```
instance-id: Atomic0
...
The actual configuration goes into `user-data` file. Let's use it to configure **ssh access** to the system. The live image is built with root password set in kickstart file:
...
It is also possible to **lock the root password** during compose of live image by modifying installation kickstart.
...
It is roughly the size defined in kicktart file

```
part / --fstype="ext4" --size=6000
```
because we run livemedia-creator with `--live-rootfs-keep-size` option. It is also possible to set the desired size directly by `--live-rootfs-size` option. We don't have to be afraid of overcommiting here as the image is sparse. But what we see here is not the real space available for writing our data. As said above, there is read-write devicemapper layer using **overlay image** (devicemapper shapshot) to store data:
...
└─docker-253:0-278707-pool   253:2    0   100G  0 dm   
  └─docker-253:0-278707-base 253:3    0    10G  0 dm   
```
The overlay image size (mounted to `loop2`) which is used for storing data written over read-only loop-mounted image (`loop1`, which is the 5G reported above) is 512M by default. So athough the root's `live-rw` device reports 3.9G available in `df`, the actual available space can be obtained by `dmsetup` command:
...
Looking at values for `live-rw` we can see that there are 632648/1048576 of 512B sectors used, which is about half of the available 512MB.
...
Let's reboot to the image with the option and pull centos docker image.
...
As we can see also from `lsblk` output and `docker info`, docker on live Atomic (unlike on regular Atomic) is using devicemapper storage backend with loopdevices which highly discouraged.
```
[fedora@atomic-00 ~]$ lsblk
NAME                         MAJ:MIN RM   SIZE RO TYPE MOUNTPOINT
sr0                           11:0    1   623M  0 rom  
...
We've recently added `iscsi-initiator-utils` to Atomic Host so now it is possible to use devicemapper backend with lvm thin pool on iSCSI device set up for docker by `docker-storage-setup` service (the service is disabled on live Atomic by kickstart command). We will use cloud-init to configure the storage, adding this configuration to `user-data`:
...
First we write configuration file for `docker-storage-service` telling it what device (`DEVS`) it should use (we are assuming diskless machine where iSCSI device will become `/dev/sda`) and how to call volume group (`VG`) the service creates for thin pool logical volume for docker.
...
And how much overlay space will pulling docker image eat now?
...
There is a new patch in rawhide dracut alowing to mount rootfs image in squashfs image in slightly different way, copying the rootrs image into RAM and loop mounting it read-write.
...
With this option there is no read-write overlay layer so you don't need to use `rd.live.overlay.size` option.
You should be able to try this dracut boot option with Atomic Host images built from rawhide repository, see next section for learning how to use another repository in place of the one embedded in installer iso. The effectivity of this approach compared to read-write overlay is something to be explored and will probably vary in aspects depending on use case.
...
With the [fedora-atomic-pxe-live.ks](https://github.com/rvykydal/anaconda-kickstarts/blob/master/atomic/fedora-atomic-pxe-live.ks) we were using Atomic repository embedded in the installer iso by including kickstart snippet from the installation image on the iso:
```
%include /usr/share/anaconda/interactive-defaults.ks
```
This snippet contains `ostreesetup` command pointing to the repository. To override it, add configuration pointing to another repo somwhere after including clause:
...
This way Atomic installer iso can be used to create live Atomic image with updated content. It is not possible to update live Atomic Host in normal way (`atomic update`) because of nature of atomic updates using ostree technology which requires rebooting into updated system (and allows for rollback to previous version of the system).


Building image with rpm-ostree-toolbox
--------------------------------------

To build Atomic repository locally [rpm-ostree-toolbox](https://github.com/projectatomic/rpm-ostree-toolbox) tool can be used. The repository is built with `treecompose` command. But using the tool also installer iso can be built with `installer` command, and there is even a `liveimage` command for building Atomic live image. It is running `livemadia-creator` in container so one big advangage is that the image can be built on other then target system without any issues. I may cover this in another blog post.
